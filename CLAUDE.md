...
# 运行指定测试文件
python3 -m pytest .claude/skills/<skill-name>/tests/test_modules.py
...
使用 `path_utils.py` 处理混合路径格式（Unix `~/` + Windows `\`）：

```python
from skill_name.path_utils import normalize_path

# 将任意输入路径规范化为正斜杠
normalized = normalize_path("~/Documents\\My Project/file.csv")
# 结果: "/home/user/Documents/My Project/file.csv"
```
