    ...
    if year:
        ...
        if granularity == "yearly":
            ...
            for q_idx, q_val in enumerate(quarterly):
                base_monthly = q_val / 3
                for m in range(1, 4):
                    results.append({
                        "period": f"{year}-Q{q_idx+1}-M{m}",
                        "value": round(base_monthly * (0.9 + 0.2 * (m / 3)), 2)
                    })
    else:
        # 不指定年份，返回最近3年趋势
        for y in sorted(metric_data.keys(), reverse=True)[:3]:
            year_data = metric_data[y]
            if granularity == "yearly":
                results.append({"period": str(y), "value": year_data.get("yearly", 0)})
            elif granularity == "quarterly" and quarter:
                quarterly = year_data.get("quarterly", [])
                idx = quarter - 1
                if idx < len(quarterly):
                    results.append({"period": f"{y}-Q{quarter}", "value": quarterly[idx]})
